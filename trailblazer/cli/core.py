...
@click.option('-sw', '--startwith', help='start the pipeline beginning with program, see format for program in mip.pl')
@click.argument('family', required=False)
@click.pass_context
def start(context, mip_config, email, priority, dryrun, command, startwith, family):
    """Start a new analysis."""
    mip_cli = MipCli(context.obj['script'])
    mip_config = mip_config or context.obj['mip_config']
    email = email or environ_email()
    kwargs = dict(config=mip_config, family=family, priority=priority, email=email, dryrun=dryrun, startwith=startwith)
    if command:
