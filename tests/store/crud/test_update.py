...
def test_add_pending_analysis(raw_analyses: List[dict], store: MockStore, user_email: str):
    ...
    unprocessed_analysis: dict = raw_analyses[0]

    # WHEN adding a new analysis
    new_analysis: Analysis = store.add_pending_analysis(
        case_id=unprocessed_analysis.get("family"),
        config_path=unprocessed_analysis.get("config_path"),
        email=user_email,
        out_dir=unprocessed_analysis.get("out_dir"),
        priority=unprocessed_analysis.get("priority"),
        type=unprocessed_analysis.get("type"),
    )
    ...
    stored_analysis: Analysis = store.get_analysis(
        ...
        status=unprocessed_analysis.get("status"),
    )
    assert new_analysis
